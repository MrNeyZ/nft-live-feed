...
/** 6-char labels (MALLOW) — one notch smaller again than LONG_LABELS, same
 *  as the table (`MintsSourceBadge.tsx`) — see `.vl-srcchip--xlong`. */
...
// Compact TXS: exact below 1K, then K (532 · 6.9K · 14K). The backend's
// bounded scan caps at 25,000, so 25,000 means "≥25K" → "25K+" (very active
// wallet, no fake precision for whales).
...
export function LiveMintFeedCard({ event: ev, group, now, paymentTokens, dimmed = false, embedded = false, onPauseEnter, onPauseLeave }: Props) {
  ...
  // CSS ellipsis); this is a manual cap restoring the width users historically
  // saw: laptop = 13 chars, PC = 16 (round(13 × 1.25), ~25% more). CSS ellipsis
  // is retained below as a safety fallback. Applied ONLY to a real per-NFT
  ...
  return (
    ...
    >
      ...
      <span title={isCollectionCreate ? undefined : priceTitle} style={{
        ...
        fontSize: isCollectionCreate ? 12 : 14, fontWeight: 800, color: isCollectionCreate ? 'var(--vl-white)' : priceColor,
