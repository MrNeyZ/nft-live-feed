...
// OFFER badge removed (Stage 4.5): it rendered `offerDelta`, which was
// silently always null in production because the backend's ME collection
// offers source (`GET /v2/collections/{slug}/offers`) is permanently dead
// (HTTP 400, no replacement — see the Jul 2026 discovery audit). The
// backend no longer computes offerDelta at all (enrich.ts); the field stays
// on the wire model as always-null for compatibility. Not repointed to
// MMM/Tensor here — the new normalized-bid model
// (src/analytics/normalized-collection-bid.ts) intentionally marks both
// venues unsuitable to independently drive a per-sale badge like this one
// (MMM eligibility/funding must be verified per pool; Tensor is
// corroboration-only) until a future stage explicitly wires that up.
        ...
        <div style={FC_RIGHT_COL_STYLE}>
          ...
          <div className="feed-price-row" style={FC_PRICE_ROW_STYLE}>
            ...
            {event.resizeStatus === 'metaplex_resized_unclaimed' && (
              ...
              // Stays gray/muted (unchanged color) — only the shape
              // (transparent+border → filled pill) changed.
              <span
